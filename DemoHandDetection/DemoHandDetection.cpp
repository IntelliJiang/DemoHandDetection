// DemoHandDetection.cpp : Defines the entry point for the console application.
//
...
int main( int argc, char** argv )
{
	...
	//this is going to set the maximum points or background image model for the ref also called Background Subtraction
	model->modMin[0] = 3;
	...
	//if filename is not there than initialize the webcam to capute image
	if( !filename )
	{
	...
	for(;;)
	{
		if( !pause )
		{
			//for each for loop query frame is going to capture present image frame and store in pointer of image
		...
		if( nframes == 1 && rawImage )
		{
			...
			yuvImage = cvCloneImage(rawImage);//create copy of rawimage
			ImaskCodeBook = cvCreateImage( cvGetSize(rawImage), IPL_DEPTH_8U, CV_LOAD_IMAGE_COLOR );//load colored imageimage is used to create temporaly IplImages of the same size.
			ImaskCodeBookCC = cvCreateImage( cvGetSize(rawImage), IPL_DEPTH_8U, CV_LOAD_IMAGE_COLOR);//load colored image
			cvSet(ImaskCodeBook,cvScalar(255));//here it is going to createspace for the multi-dimensiona
		...
		if( rawImage )
		{
			...
			if( !pause && nframes-1 < nframesToLearnBG  )
				cvBGCodeBookUpdate( model, yuvImage );//first time build you max fingers model
